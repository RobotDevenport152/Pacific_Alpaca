...
export default function ComparePage() {
  ...
  // FIX: derive prices from live DB instead of hardcoded strings
  const { data: dbProducts } = useProducts();

  function getLivePrice(tierId: string): string {
    const p = dbProducts?.find(x => x.id === tierId);
    if (!p) return '...';
    return locale === 'zh'
      ? `¥${p.prices.CNY.toLocaleString()} 起`
      : `From NZD$${p.prices.NZD.toLocaleString()}`;
  }
              ...
              <div key={tier.id} className={`bg-card rounded-lg border p-5 space-y-4 ${tier.highlight ? 'border-gold/40' : 'border-border'}`}>
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-muted-foreground text-xs font-body mt-0.5">{getLivePrice(tier.id)}</p>
                  </div>
                  {tier.highlight && (
