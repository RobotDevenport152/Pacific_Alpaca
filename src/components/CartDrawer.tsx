              ...
              <div key={`${item.product.id}-${item.variant}`} className="flex gap-4 bg-card rounded-lg p-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    {/* FIX: cap quantity at available stock to prevent over-ordering */}
                    <button
                      onClick={() => updateQuantity(item.product.id, Math.min(item.quantity + 1, item.product.stock), item.variant)}
                      disabled={item.quantity >= item.product.stock}
                      className="w-7 h-7 rounded border border-border flex items-center justify-center hover:bg-muted disabled:opacity-40 disabled:cursor-not-allowed"
                    >
