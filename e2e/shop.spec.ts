...
test.describe('Shop Page', () => {
  ...
  test('search filters product list', async ({ page }) => {
    await page.locator('.group.bg-card').first().waitFor({ timeout: 15_000 });
    const title = (await page.locator('.group.bg-card h3').first().textContent())?.trim() ?? '';
    const q = title.length >= 2 ? title.slice(0, 2) : 'a';
    await page.fill('input[placeholder*="Search"], input[placeholder*="搜索"]', q);
...
test.describe('Product Detail Page', () => {
  ...
  test('product detail shows name, price, and add-to-cart button', async ({ page }) => {
    ...
    await expect(page.locator('.text-gold.font-display').filter({ hasText: /\d/ }).first()).toBeVisible({
      timeout: 10_000,
    });
