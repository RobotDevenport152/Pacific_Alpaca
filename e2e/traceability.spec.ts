...
test.describe('Compare Page', () => {
  test('/compare renders all three tiers', async ({ page }) => {
    ...
    await expect(page.getByText(/经典款|Classic/i).first()).toBeVisible({ timeout: 8_000 });
    await expect(page.getByText(/轻奢款|Luxury/i).first()).toBeVisible({ timeout: 5_000 });
    await expect(page.getByText(/高奢款|Premium/i).first()).toBeVisible({ timeout: 5_000 });
  });
...
test.describe('Returns Page', () => {
  test('/returns page loads and shows 30-day policy', async ({ page }) => {
    ...
    await expect(page.getByText(/30天|30-Day/i).first()).toBeVisible({ timeout: 8_000 });
  });

  test('returns page has contact email link', async ({ page }) => {
    await page.goto(`${BASE_URL}/returns`);
    await expect(page.getByRole('link', { name: /returns@pacificalpacas/i })).toBeVisible({ timeout: 5_000 });
  });
});
