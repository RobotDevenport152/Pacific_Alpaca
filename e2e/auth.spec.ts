...
test.describe('Authentication', () => {
  ...
  test('register page renders all required fields', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: /register|注册|Create Account/i })).toBeVisible();
  });

  test('register with existing email shows error', async ({ page }) => {
    await page.goto(`${BASE_URL}/register`);
    await page.locator('input[type="text"]').first().fill('E2E Existing User');
    await page.fill('input[type="email"]', TEST_USERS.regular.email);
    await page.locator('input[type="password"]').nth(0).fill(TEST_USERS.regular.password);
    await page.locator('input[type="password"]').nth(1).fill(TEST_USERS.regular.password);
  ...
  test('register with invalid email shows validation error', async ({ page }) => {
    await page.goto(`${BASE_URL}/register`);
    await page.locator('input[type="text"]').first().fill('E2E');
    await page.fill('input[type="email"]', 'not-an-email');
    await page.locator('input[type="password"]').nth(0).fill('somepassword1');
    await page.locator('input[type="password"]').nth(1).fill('somepassword1');
    await page.click('button[type="submit"]');
